...
def split():
    if request.method == 'POST':
        ...
        # Create a list of the split page file paths
        page_paths = [os.path.join(output_dir, f'page_{i+1}.pdf') for i in range(num_pages)]
        
        # Create a new zip file and add the split page files to it
        zip_path = os.path.join(app.config['UPLOAD_FOLDER'], 'output.zip')
        with zipfile.ZipFile(zip_path, 'w') as zip_file:
            for page_path in page_paths:
                zip_file.write(page_path, os.path.basename(page_path))
        print(zip_path)
        print(os.path.basename(zip_path))
        # Return the zip file as a download
        #send_from_directory(directory, path, filename=None, **kwargs)
